...
# v2 Phan 9: anh xa trang thai Application -> loai Email Template tu dong
# kich hoat. INTERVIEW_INVITATION gan vao luc chuyen sang INTERVIEW (chua co
...
# v2 Phan 3.2: canh chuyen forward hop le do HR/HR Manager/Admin thuc hien qua
...
# DRAFT->PENDING_APPROVAL->APPROVED - dung quy trinh 4-eyes CHANGE 01), KHONG
# duoc phep set truc tiep qua endpoint nay - neu khong HR co the "gia mao"
# status=OFFER ma khong he co Offer nao duoc tao/duyet, pha vo toan bo 4-eyes
# approval va lam ket hon so (khong con cach nao tao Offer that vi UI/nghiep
# vu chi cho tao Offer khi status dang la INTERVIEW). Tuong tu, HIRED/REJECTED
# tu OFFER do offer_service quan ly (respond_offer/expire_due_offers), khong
# di qua ham update_status nay.
#
...
# nhanh "AI khong kha dung -> can sang loc thu cong" da thiet ke o v2 Phan 8.5,
# chi khac la o day no la truong hop thuong truc (chua co AI that) chu khong
# phai truong hop loi tam thoi. Khi AI Screening service duoc xay that, canh
# nay VAN giu nguyen lam fallback (dung 8.5), chi thay doi la se co them mot
# duong system-driven NEW->AI_SCREENING->SCREENING chay song song.
...
def apply_for_job(
    ...
    """CHANGE 02: tao Application moi cho moi lan Apply hop le, KHONG kiem tra
    cooldown 90-ngay o bat ky dau trong ham nay (da bo hoan toan theo yeu cau).
    Application cu (neu co, vi du REJECTED/WITHDRAWN) duoc giu nguyen, khong
    overwrite. Tra ve (application, is_new): is_new=False khi trung
    Idempotency-Key voi 1 request truoc do cua chinh candidate nay.
    """
    if idempotency_key:
        existing = (
    ...
    # BUG FIX (theo yeu cau nguoi dung): chan nop THEM ho so vao CUNG 1 tin
    # neu candidate dang co san 1 Application khac cho tin nay da qua buoc
    # sang loc (tu SCREENING tro di) va CHUA ket thuc - tranh 2 ho so trung
    # nhau cung tin cung duoc xu ly song song, gay nham lan cho HR. KHONG ap
    # dung cho Application con o NEW/AI_SCREENING (chua sang loc) hay da ket
    # thuc (REJECTED/WITHDRAWN/ARCHIVED) - van giu dung tinh than "tai ung
    # tuyen khong gioi han" (CHANGE 02) cho cac truong hop do, chi chan khi
    # thuc su co 1 ho so con "song" dang duoc xu ly dang do.
    active_application = (
        db.query(Application)
        .filter(
            Application.candidate_id == candidate.id,
    ...
    application = Application(
        ...
        job_id=job.id,
        # v2.3 Section 9.3/9.4/9.5: source_id la FK "song" vao candidate_sources
        # (rename/disable/reporting dung theo master data); source (text) la
        # snapshot ten nguon tai thoi diem nop don, khong doi ngay ca khi
        # CandidateSource sau nay bi doi ten.
        ...
        # scope "HR chi xu ly Application duoc gan" (Rule 5) khong bao gio dung
        # vi assigned_hr_id luon NULL cho moi Application moi tao.
        assigned_hr_id=job.assigned_hr_id,
        # v2.2 Section 3: Job Department = Application Department mac dinh,
        # khong cho form override tu do.
...
def update_status(
    ...
    """Validate transition theo bang 3.2 truoc khi ghi DB (giai quyet CONFLICT
    Kanban keo-tha-tu-do vs state machine 1 chieu - v2 Phan 3.3).
    """
...
def archive(
    ...
    """v2 Phan 3.2: 'Bat ky (tru HIRED) -> ARCHIVED | HR Manager / Admin'.
    Phat hien thieu khi doi chieu code voi spec - truoc ban va nay khong co
    duong nao dat toi trang thai ARCHIVED du field archive_reason da co san
    trong DB tu dau.
    """
...
def update_application(
    ...
    """v2.3 Section 5/18 - PUT /applications/{id}: sua Luong mong muon va
    Nguon ho so. KHONG cho doi job_id/candidate_id qua day (Section 5 - tranh
    pha vo relationship co dinh; doi Job can nghiep vu rieng phuc tap hon).
    """
    before = {
